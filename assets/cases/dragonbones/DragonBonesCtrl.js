...
// runtime not support dragonbones.
    ...
    let NORMAL_ANIMATION_GROUP = 'normal';
    let AIM_ANIMATION_GROUP = 'aim';
    let ATTACK_ANIMATION_GROUP = 'attack';
    let JUMP_SPEED = -20;
    let NORMALIZE_MOVE_SPEED = 3.6;
    let MAX_MOVE_SPEED_FRONT = NORMALIZE_MOVE_SPEED * 1.4;
    let MAX_MOVE_SPEED_BACK = Number(NORMALIZE_MOVE_SPEED);
    let WEAPON_R_LIST = ['weapon_1502b_r', 'weapon_1005', 'weapon_1005b', 'weapon_1005c', 'weapon_1005d', 'weapon_1005e'];
    let WEAPON_L_LIST = ['weapon_1502b_l', 'weapon_1005', 'weapon_1005b', 'weapon_1005c', 'weapon_1005d'];
    let GROUND = -200;
    let G = -0.6;
    ...
    cc.Class({
        ...
        switchWeaponL: function() {
            ...
            let newWeaponName = WEAPON_L_LIST[this._weaponLIndex];
        ...
        switchWeaponR: function() {
            ...
            let newWeaponName = WEAPON_R_LIST[this._weaponRIndex];
        ...
        _frameEventHandler: function (event) {
            if (event.name === 'onFire') {
                let firePointBone = event.armature.getBone('firePoint');
                let localPoint = cc.v2(firePointBone.global.x, -firePointBone.global.y);

                let display = event.armature.display;
                let globalPoint = display.node.convertToWorldSpace(localPoint);
        ...
        _fire: function (firePoint) {
            ...
            let armature = this._armatureDisplay.buildArmature('bullet_01');
            let effect = this._armatureDisplay.buildArmature('fireEffect_01');
            let radian = this._faceDir < 0 ? Math.PI - this._aimRadian : this._aimRadian;
            let bullet = new DragonBullet();
        ...
        _updatePosition: function() {
            if (this._speedX !== 0) {
                ...
                let minX = -cc.visibleRect.width / 2;
                let maxX = cc.visibleRect.width / 2;
                if (this.node.x < minX) {
                    this.node.x = minX;
        ...
        _updateAim: function () {
            ...
            let aimOffsetY = this._armature.getBone('chest').global.y * this.node.scaleY;
    ...
    var DragonBullet = cc.Class({
        ...
        init: function (parentNode, armature, effect, radian, speed, position) {
            ...
            let thePos = parentNode.convertToNodeSpaceAR(position);
            ...
            if (effect) {
                this._effect = effect;
                let effectDisplay = this._effect.node;
                effectDisplay.rotation = radian * cc.macro.DEG;
                effectDisplay.setPosition(thePos);
                effectDisplay.scaleX = 1 + Number(Math.random());
        ...
        update: function() {
            ...
            let worldPos = armatureNode.parent.convertToWorldSpaceAR(armatureNode.getPosition());
            if (
