...
cc.Class({
    ...
    createBezierNodes: function () {
        ...
        for (let quantity = 0, len = 6; quantity < len; quantity++) {

            let theta = Math.PI * 2 * quantity / len;

            let x = cc.winSize.width * 0.5 + 0 * Math.cos(theta);
            let y = cc.winSize.height * 0.5 + 0 * Math.sin(theta);
    ...
    update: function (dt) {
        ...
        [].forEach.call(nodes, (node, index) => {
            ...
            if (this.input) node.disturb += (node.max - node.disturb) * this.reactivity;
    ...
    render: function () {
        ...
        let currentIndex; let nextIndex; let xc; let
            yc;
